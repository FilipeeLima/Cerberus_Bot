...
function salvarConfig() {
    // Função para salvar a configuração usando o API de armazenamento local do Chrome
...
function carregarConfiguracao() {
    // Função para carregar a configuração previamente salva
    chrome.storage.local.get(["configuracao"], (res) => {
        // Se a configuração não existir, define alguns valores padrão e salva
        if (res.configuracao == undefined) {
            ...
            // Se a configuração existir, atribui os valores às variáveis correspondentes
...
function inserirTextoDisplay(texto, tela) {
    // Função para inserir texto no display específico
    if (tela == 1) {
...
function analisandoEstrategias() {
    ...
    if (qtdRoletas > 1) {
        ...
        if (!displayLobbyExists) {
            ...
            painelLobby.insertAdjacentHTML('afterbegin', '<h1 id = "displaybotlobby"  style="width: 90%;background-color: #DAD4B5;color: #952323;text-align: center; font-size: xx-large;font-weight: bolder;align-self: center;"></h1>');
            displayRoletaExists = false
            displayLobbyExists = true
        }

        inserirTextoDisplay(`🤖 CERBERUS BOT(ANALISANDO ENTRADAS) - ${contagemAcertos} WIN - ${contagemErros} LOSS 🤖`, 1)
